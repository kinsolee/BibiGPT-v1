...
import {
  ...
  buildSyntheticChunkVideoId,
  buildSyntheticReduceVideoId,
...
// ---------- 42. 合成 provider 缓存身份：内容变化即分键 ----------
{
  const videoId = 'BV1cache'
  const c1 = buildSyntheticChunkVideoId(videoId, 0, 'aaaabbbbccccdddd')
  const c2 = buildSyntheticChunkVideoId(videoId, 0, 'ddddccccbbbbaaaa')
  const c3 = buildSyntheticChunkVideoId(videoId, 1, 'aaaabbbbccccdddd')
  assert(c1 !== c2, 'cache-id: 同 chunk 序号内容变化即分键')
  assert(c1 !== c3, 'cache-id: 不同 chunk 序号分键')
  assert(c1.endsWith('#chunk0-aaaabbbbccccdddd'), 'cache-id: chunk hash 编入合成 videoId')

  const chunksA = [{ hash: 'h1' }, { hash: 'h2' }]
  const chunksB = [{ hash: 'h1' }, { hash: 'h3' }]
  const r1 = buildSyntheticReduceVideoId(videoId, chunksA, '标题')
  const r2 = buildSyntheticReduceVideoId(videoId, chunksB, '标题')
  const r3 = buildSyntheticReduceVideoId(videoId, chunksA, '新标题')
  const r4 = buildSyntheticReduceVideoId(videoId, chunksA, ' 新标题 ')
  assert(r1 !== r2, 'cache-id: chunk 集合变化 → reduce 分键')
  assert(r1 !== r3 && r3 === r4, 'cache-id: 标题变化分键、规范化后一致')
}

// ---------- 43. 异步入队 onCompleted：完成（含复用）后触发回调 ----------
{
  const store = new MemoryJobStore()
  const engine = makeEngine(store, recordingRunner({ chunkDelayMs: 60 }))
  const params = buildParams(syntheticItems(10, { bytesPerItem: 300 }))
  const results = []
  await startSummaryJobInBackground(params, {
    engine,
    onCompleted: async (result) => {
      results.push(result.summaryText)
    },
  })
  await sleep(400)
  assert(results.length === 1 && results[0].includes('## Summary'), 'onCompleted: 完成后收到摘要')
  // 二次入队走复用路径，同样触发回调
  await startSummaryJobInBackground(params, {
    engine,
    onCompleted: async (result) => {
  ...
  await sleep(300)
  assert(results.length === 2, 'onCompleted: 复用路径也触发')
}
